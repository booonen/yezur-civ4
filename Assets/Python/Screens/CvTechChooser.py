...
FLAVORS = [ TechPrefs.FLAVOR_PRODUCTION, TechPrefs.FLAVOR_GOLD, TechPrefs.FLAVOR_SCIENCE,
			TechPrefs.FLAVOR_CULTURE, TechPrefs.FLAVOR_RELIGION, TechPrefs.FLAVOR_ESPIONAGE, TechPrefs.FLAVOR_MILITARY ]
UNIT_CLASSES = [ "UNITCLASS_ENGINEER", "UNITCLASS_MERCHANT", "UNITCLASS_SCIENTIST",
				 "UNITCLASS_ARTIST", "UNITCLASS_PROPHET", "UNITCLASS_GREAT_SPY", "UNITCLASS_GREAT_GENERAL" ]
...
# BUG - GP Tech Prefs - end
		...
		self.BOX_INCREMENT_WIDTH = 57 # Used to be 33 #Should be a multiple of 3...
		self.BOX_INCREMENT_HEIGHT = 9 #Should be a multiple of 3...
		self.BOX_INCREMENT_Y_SPACING = 6 #Should be a multiple of 3...
		self.BOX_INCREMENT_X_SPACING = 6 #Should be a multiple of 3...
...
# BUG - Tech Screen Resolution - start
		...
		screen.setDimensions((screen.getXResolution() - xPanelWidth) / 2, screen.centerY(0) - 128, xPanelWidth, yPanelHeight)
...
# BUG - Tech Era Colors - start
			...
			iShadowOffset = 9
			screen.attachPanelAt( sPanel, szTechRecordShadow, u"", u"", True, False, PanelStyles.PANEL_STYLE_TECH, iX - 6 + iShadowOffset, iY - 6 , self.getXStart() + 6, 12 + ( self.BOX_INCREMENT_HEIGHT * self.PIXEL_INCREMENT ), WidgetTypes.WIDGET_TECH_CHOOSER_ERA, gc.getTechInfo(i).getEra(), -1 )
...
# BUG - Tech Era Colors - end
			...
			if ( gc.getTeam(gc.getPlayer(self.iCivSelected).getTeam()).isHasTech(i) ):
				...
				screen.setPanelColor(szTechRecord, 45, 151, 163)
				...
				# Ancient Era
				if ( gc.getTechInfo(i).getEra()%2 == 0 ):
					screen.setPanelColor(szTechRecord, 160, 64, 160)
				# Post-Human Era
				else:
					screen.setPanelColor(szTechRecord, 160, 140, 64)
			...
			if bTechIcon:
				...
				screen.addDDSGFCAt( szTechButtonID, szTechRecord, gc.getTechInfo(i).getButton(), iX + 6, iY + 6, TEXTURE_SIZE*2, TEXTURE_SIZE*2, WidgetTypes.WIDGET_TECH_TREE, i, -1, False )

			if bTechDetails:
				self.addIconsToTechPanel(screen, i, (X_START + (TEXTURE_SIZE*2)), iX, iY, szTechRecord)
...
# BUG - 3.19 No Espionage - end
		...
		if ( gc.getTechInfo(i).isIrrigation() ):
			...
			screen.addDDSGFCAt( szIrrigationButton, szTechRecord, ArtFileMgr.getInterfaceArtInfo("INTERFACE_TECH_RURALISM").getPath(), iX + fX, iY + Y_ROW, TEXTURE_SIZE, TEXTURE_SIZE, WidgetTypes.WIDGET_HELP_IRRIGATION, i, -1, False )
			fX += X_INCREMENT
