...
class CvCivicsScreen:
	...
	def setValues(self):
		...
		self.PANEL_WIDTH = 0
		self.INITIAL_SPACING = 5
		self.HEADINGS_WIDTH = 200
		for k in range(gc.getNumCivicOptionInfos()):
			self.CIVICCATEGORIES = k
		for l in range(gc.getNumCivicInfos()):
			self.HEADINGS_HEIGHT = (((40 + self.BUTTON_SIZE + self.TEXT_MARGIN) * l/self.CIVICCATEGORIES)/5 * 2) + 180
