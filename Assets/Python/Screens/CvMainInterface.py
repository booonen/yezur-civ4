					...
					if (pPlayer.isCommerceFlexible(eCommerce) or (CyInterface().isCityScreenUp() and (eCommerce == CommerceTypes.COMMERCE_GOLD))):
						...
						if not CyInterface().isCityScreenUp():
							...
							if eCommerce == CommerceTypes.COMMERCE_RESEARCH:
								...
								if iTech > -1:
									iBonusRate = TechResource.calculateModifier(iTech, ePlayer)
									if iBonusRate != 0:
										sColor = CyTranslator().getText("[COLOR_WARNING_TEXT]", ())
										if iBonusRate > 0:
											sColor = CyTranslator().getText("[COLOR_POSITIVE_TEXT]", ())
										szOutText += u"<font=2>%s %+d%%</color></font>" %(sColor, iBonusRate)
							szString = "RateText" + str(iI)
# BUG - Min/Max Sliders - start
...
## Unique Promotions ##
							sPromo = gc.getPromotionInfo(i).getType()
							if sPromo.startswith("PROMOTION_CIVILIZATION"): continue
## Unique Promotions ##
							szName = "PromotionButton" + str(i)
