...
## Unique Promotions ##
		...
		if (ePromo > -1):
			sPromo = gc.getPromotionInfo(ePromo).getType()
			if sPromo.startswith("PROMOTION_CIVILIZATION"):
				iCiv = gc.getInfoTypeForString(sPromo[sPromo.find("CIVILIZATION"):])
				if iCiv > -1:
		...
		if (ePromoOr1 > -1):
			...
			sPromo = gc.getPromotionInfo(ePromoOr1).getType()
			if sPromo.startswith("PROMOTION_CIVILIZATION"):
				iCiv = gc.getInfoTypeForString(sPromo[sPromo.find("CIVILIZATION"):])
				if iCiv > -1:
					screen.attachImageButton( panelName, "", gc.getCivilizationInfo(iCiv).getButton(), GenericButtonSizes.BUTTON_SIZE_CUSTOM, WidgetTypes.WIDGET_PEDIA_JUMP_TO_CIV, iCiv, 1, False )
			...
			if (ePromoOr2 > -1):
				...
				sPromo = gc.getPromotionInfo(ePromoOr2).getType()
				if sPromo.startswith("PROMOTION_CIVILIZATION"):
					iCiv = gc.getInfoTypeForString(sPromo[sPromo.find("CIVILIZATION"):])
					if iCiv > -1:
						screen.attachImageButton( panelName, "", gc.getCivilizationInfo(iCiv).getButton(), GenericButtonSizes.BUTTON_SIZE_CUSTOM, WidgetTypes.WIDGET_PEDIA_JUMP_TO_CIV, iCiv, 1, False )
