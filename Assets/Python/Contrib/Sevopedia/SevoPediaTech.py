...
class SevoPediaTech(CvPediaScreen.CvPediaScreen):
	...
	def __init__(self, main):
		...
		self.H_SPECIAL_PANE = self.top.B_PEDIA_PAGE - self.Y_SPECIAL_PANE #- self.H_PREREQ_PANE
		
		self.X_STRATEGY_PANE = self.X_TECH_PANE + self.W_TECH_PANE + 10
		self.Y_STRATEGY_PANE = self.Y_TECH_PANE
		self.W_STRATEGY_PANE = self.top.R_PEDIA_PAGE - self.X_CIVS 
		self.H_STRATEGY_PANE = 110
	...
	def placeSpecial(self):
		...
		## Tech Resource ##
		sTechType = gc.getTechInfo(self.iTech).getType()
		if sTechType in TechResource.TechBonus:
			for (sBonus, iRate) in TechResource.TechBonus[sTechType]:
				iBonus = gc.getInfoTypeForString(sBonus)
				if iBonus > -1 and iRate != 0:
					szSpecialText += localText.getText("TXT_KEY_TECH_BONUS_MODIFIER", (iRate, gc.getBonusInfo(iBonus).getDescription(),))
		## Tech Resource ##
	...
	def placeStrategy(self):
		screen = self.top.getScreen()
		panelName = self.top.getNextWidgetName()
		screen.addPanel(panelName, localText.getText("Strategy", ()), "", True, False, self.X_STRATEGY_PANE, self.Y_STRATEGY_PANE, self.W_STRATEGY_PANE, self.H_STRATEGY_PANE, PanelStyles.PANEL_STYLE_BLUE50)
		screen.attachLabel(panelName, "", "  ")
