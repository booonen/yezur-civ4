...
## Requirement Script ##
		for item in Requirement.lBuildings:
			for i in item[0]:
				if i != self.iBuilding: continue
				if item[2]: szSpecialText += CyTranslator().getText("TXT_KEY_REQUIRES_POP", (item[2],))
				if item[3] > -1: szSpecialText += CyTranslator().getText("TXT_KEY_REQUIRES_CULTURE", (gc.getCultureLevelInfo(item[3]).getDescription(),))
				if item[14]: szSpecialText += CyTranslator().getText("TXT_KEY_REQUIRES_BUILT_ON_HILLS", ())
				if item[15]: szSpecialText += CyTranslator().getText("TXT_KEY_REQUIRES_NATIONALITY", (item[15],))
				if item[16]: szSpecialText += CyTranslator().getText("TXT_KEY_REQUIRES_WAR", ())
				if item[17]: szSpecialText += CyTranslator().getText("TXT_KEY_REQUIRES_PEACE", ())
				for j in xrange(len(item[18])):
					if j == 0:
						szSpecialText += CyTranslator().getText("TXT_KEY_CANNOT_HAVE_BUILDING", ()) + " "
					szSpecialText += gc.getBuildingInfo(item[18][j]).getDescription()
					if j < len(item[18]) - 1:
						szSpecialText += CyTranslator().getText("TXT_KEY_OR", ())
				for j in xrange(len(item[1])):
					if j == 0:
						szSpecialText += CyTranslator().getText("TXT_KEY_REQUIRES_CIVICS", ()) + " "
					szSpecialText += gc.getCivicInfo(item[1][j]).getDescription()
					if j < len(item[1]) - 1:
						szSpecialText += CyTranslator().getText("TXT_KEY_OR", ())
				if item[4]: szSpecialText += CyTranslator().getText("TXT_KEY_REQUIRES_VICINITY", ()) + " " + gc.getTerrainInfo(gc.getInfoTypeForString("TERRAIN_HILLS")).getDescription()
				if item[5]: szSpecialText += CyTranslator().getText("TXT_KEY_REQUIRES_VICINITY", ()) + " " + gc.getTerrainInfo(gc.getInfoTypeForString("TERRAIN_PEAK")).getDescription()


				if item[6] > -1:
					szSpecialText += CyTranslator().getText("TXT_KEY_REQUIRES_VICINITY", ()) + " " + gc.getTerrainInfo(item[6]).getDescription()
					if len(item[7]):
						szSpecialText += CyTranslator().getText("TXT_KEY_AND", ())
						if len(item[7]) > 1:
							szSpecialText += "( "
				for j in xrange(len(item[7])):
					if j == 0 and item[6] < 0:
						szSpecialText += CyTranslator().getText("TXT_KEY_REQUIRES_VICINITY", ()) + " "
					szSpecialText += gc.getTerrainInfo(item[7][j]).getDescription()
					if j < len(item[7]) - 1:
						szSpecialText += CyTranslator().getText("TXT_KEY_OR", ())
				if item[6] > -1 and len(item[7]) > 1:
					szSpecialText += ")"

				if item[8] > -1:
					szSpecialText += CyTranslator().getText("TXT_KEY_REQUIRES_VICINITY", ()) + " " + gc.getFeatureInfo(item[8]).getDescription()
					if len(item[9]):
						szSpecialText += CyTranslator().getText("TXT_KEY_AND", ())
						if len(item[9]) > 1:
							szSpecialText += "( "
				for j in xrange(len(item[9])):
					if j == 0 and item[8] < 0:
						szSpecialText += CyTranslator().getText("TXT_KEY_REQUIRES_VICINITY", ()) + " "
					szSpecialText += gc.getFeatureInfo(item[9][j]).getDescription()
					if j < len(item[9]) - 1:
						szSpecialText += CyTranslator().getText("TXT_KEY_OR", ())
				if item[8] > -1 and len(item[9]) > 1:
					szSpecialText += ")"

				if item[10] > -1:
					szSpecialText += CyTranslator().getText("TXT_KEY_REQUIRES_VICINITY", ()) + " " + gc.getBonusInfo(item[10]).getDescription()
					if len(item[11]):
						szSpecialText += CyTranslator().getText("TXT_KEY_AND", ())
						if len(item[11]) > 1:
							szSpecialText += "( "
				for j in xrange(len(item[11])):
					if j == 0 and item[10] < 0:
						szSpecialText += CyTranslator().getText("TXT_KEY_REQUIRES_VICINITY", ()) + " "
					szSpecialText += gc.getBonusInfo(item[11][j]).getDescription()
					if j < len(item[11]) - 1:
						szSpecialText += CyTranslator().getText("TXT_KEY_OR", ())
				if item[10] > -1 and len(item[11]) > 1:
					szSpecialText += ")"

				if item[12] > -1:
					szSpecialText += CyTranslator().getText("TXT_KEY_REQUIRES_VICINITY", ()) + " " + gc.getImprovementInfo(item[12]).getDescription()
					if len(item[13]):
						szSpecialText += CyTranslator().getText("TXT_KEY_AND", ())
						if len(item[13]) > 1:
							szSpecialText += "( "
				for j in xrange(len(item[13])):
					if j == 0 and item[12] < 0:
						szSpecialText += CyTranslator().getText("TXT_KEY_REQUIRES_VICINITY", ()) + " "
					szSpecialText += gc.getImprovementInfo(item[13][j]).getDescription()
					if j < len(item[13]) - 1:
						szSpecialText += CyTranslator().getText("TXT_KEY_OR", ())
				if item[12] > -1 and len(item[13]) > 1:
					szSpecialText += ")"
## Requirement Script ##
